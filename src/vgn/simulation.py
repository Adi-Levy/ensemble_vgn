...
class ClutterRemovalSim(object):
    ...
    def reset(self, object_count, first_run=True):
        self.world.reset()
        ...
        if self.scene == "pile":
            self.generate_pile_scene(object_count, table_height, first_run)
    ...
    def draw_poc(self, poc):
        color = [1, 0, 0]
        self.pocs.append(self.world.p.addUserDebugLine(lineFromXYZ=poc, lineToXYZ=[0,0,0], lineColorRGB=color))

    def erase_pocs(self):
        for poc in self.pocs:
            self.world.p.removeUserDebugItem(itemUniqueId=poc)
    ...
    def generate_pile_scene(self, object_count, table_height, first_run=True):
        # place box
        ...
        if first_run:
            ...
            for urdf in urdfs:
                ...
                self.urdfs_data.append((urdf, pose, scale))
        for urdf, pose, scale in self.urdfs_data:
